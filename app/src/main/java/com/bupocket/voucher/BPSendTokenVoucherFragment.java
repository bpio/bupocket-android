    ...
    private void getAccountAvailableTokenBalance(String tokenType, String tokenBalance) {
        ...
        if (TokenTypeEnum.BU.getCode().equals(tokenType)) {
            if (tokenBalance == null || Double.parseDouble(tokenBalance) < 0 || Double.parseDouble(tokenBalance) == 0) {
                ...
                Double doubleAvailableTokenBalance = DecimalCalculate.sub(Double.parseDouble(tokenBalance), Constants.RESERVE_AMOUNT);
                if (doubleAvailableTokenBalance < 0) {
                    availableTokenBalance = "0";
                } else {
                    availableTokenBalance = CommonUtil.rvZeroAndDot(new BigDecimal(DecimalCalculate.sub(Double.parseDouble(tokenBalance), Constants.RESERVE_AMOUNT)).setScale(Integer.valueOf(tokenDecimals), BigDecimal.ROUND_HALF_UP).toPlainString());
                }
    ...
    private void confirmSendInfo() {
        ...
        mConfirmSendBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (Double.parseDouble(sendAmountInput) < Constants.MIN_SEND_AMOUNT) {
                    tipDialog = new QMUITipDialog.Builder(getContext())
                ...
                if ((TokenTypeEnum.BU.getCode().equals(tokenType)) && (Double.parseDouble(sendAmountInput) > Constants.MAX_SEND_AMOUNT)) {
                    tipDialog = new QMUITipDialog.Builder(getContext())
